...
function BSFieldsForm({ form }: { form: UseFormReturn<Omit<BSParams, "type">> }) {
  return (
    <div className="grid grid-cols-1 gap-6">
      {["spot", "strike", "rate", "volatility", "maturity"].map((name) => {
...
export default function BSForm({ onResult }: { onResult?: (res: DualPricing) => void }) {
  ...
  const lastSentRef = useRef<Partial<BSParams> | null>(null);
  const debounceRef = useRef<number | null>(null);

  useEffect(() => {
    const params = { spot, strike, rate, volatility, maturity };

    if (lastSentRef.current && paramsEqual(lastSentRef.current, params)) {
      return;
    }
  ...
  }, [spot, strike, rate, volatility, maturity, onResult]);
